<div align="center">

# ⚡ Ismaeel Shaikh ⚡

### 🚀 Serverless & GenAI Engineer | Full-Stack Developer | AWS CDK | RAG Specialist

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&size=22&pause=1000&color=00D9FF&center=true&vCenter=true&width=600&lines=Building+AI-Powered+Apps+at+Scale;AWS+Serverless+%7C+GenAI+%7C+Full-Stack;Shortlisted+%40+AI+for+Bharat+%F0%9F%8F%86;Always+Learning%2C+Always+Shipping+%F0%9F%9A%80)](https://git.io/typing-svg)

</div>

---

## 🙋‍♂️ Who I Am

I build **scalable, production-grade AI applications** that solve real-world problems.

Pursuing **B.E. in Information Technology** at APSIT (Mumbai University), my focus is entirely on bridging academic theory with industry-grade engineering. I specialize in:

- 🤖 **Generative AI** — RAG pipelines, LLM orchestration, Prompt Engineering
- ☁️ **Serverless Cloud Architecture** — AWS CDK, Lambda, API Gateway, Bedrock
- 🔗 **Full-Stack Development** — React, Node.js, FastAPI, Firebase
- 🛡️ **DevOps & Security** — Docker, Jenkins, CI/CD, Nmap, Wireshark

> I thrive in environments where **speed, clarity, and real engineering impact** matter.

---

## 🛠️ Tech Stack

### 🤖 AI / ML & GenAI
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![LangChain](https://img.shields.io/badge/LangChain-1C3C3C?style=for-the-badge&logo=langchain&logoColor=white)
![Amazon Bedrock](https://img.shields.io/badge/AWS_Bedrock-FF9900?style=for-the-badge&logo=amazon-aws&logoColor=white)
![Groq](https://img.shields.io/badge/Groq-F55036?style=for-the-badge&logoColor=white)

- 🔍 RAG (Retrieval-Augmented Generation)
- 🧠 LLM Orchestration & Prompt Engineering
- 📦 Vector Databases (ChromaDB)
- 🤖 Agentic AI Workflows
- 🌐 Claude 3.5 Sonnet | Amazon Nova Pro

### ☁️ Cloud & Serverless (AWS)
![AWS](https://img.shields.io/badge/AWS-FF9900?style=for-the-badge&logo=amazon-aws&logoColor=white)
![AWS CDK](https://img.shields.io/badge/AWS_CDK-232F3E?style=for-the-badge&logo=amazon-aws&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=white)

- ⚡ Lambda | API Gateway | S3 | Cognito
- 🔄 WebSockets | CloudFront | DynamoDB
- 🏗️ Infrastructure as Code (AWS CDK)

### 🖥️ Backend Engineering
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=for-the-badge&logo=fastapi&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=cplusplus&logoColor=white)

- 🔗 REST APIs & WebSocket servers
- 🔐 JWT Auth | AWS Cognito
- 🗄️ PostgreSQL | MongoDB | MySQL | DynamoDB

### 🎨 Frontend Systems
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![Vite](https://img.shields.io/badge/Vite-646CFF?style=for-the-badge&logo=vite&logoColor=white)
![Firebase](https://img.shields.io/badge/Firebase-FFCA28?style=for-the-badge&logo=firebase&logoColor=black)

- ⚡ Real-time dashboards & event platforms
- 🔄 CI/CD automated deployments
- 📱 Zero-downtime production deployments

### 🔧 DevOps & Security
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)
![Jenkins](https://img.shields.io/badge/Jenkins-D24939?style=for-the-badge&logo=jenkins&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![Ansible](https://img.shields.io/badge/Ansible-EE0000?style=for-the-badge&logo=ansible&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)

---

## 🚀 Featured Projects

### ⚖️ Nyaya Mitra AI — Legal AI Platform
> **🏆 Shortlisted @ AI for Bharat | $100 AWS Grant Winner**

A **fully serverless legal AI application** built with AWS CDK (TypeScript).

- 🏗️ Zero-server infrastructure — auto-scales from 0 to 1000+ concurrent requests
- 🤖 AWS Bedrock (Amazon Nova Pro) for legal document summarization & letter drafting
- 🔄 Real-time communication via API Gateway WebSockets
- 🔐 Secure auth via AWS Cognito | DynamoDB + S3 persistence layer

**🔗 Live:** https://d1r7thw3rznz88.cloudfront.net/

---

### 🎓 Smart Campus Connect — AI RAG Assistant
> **Production-grade RAG system for college knowledge base**

- 🔍 Scraped & vectorized entire college website into ChromaDB
- 🧠 Multi-turn conversational memory with Groq LLM + PostgreSQL
- 🔗 Modular LangChain pipeline — retrieval, ranking & generation decoupled
- ⚡ Sub-second semantic search over institutional data

**🔗 GitHub:** [ismaeeeelshaikh/college-ai-assistant](https://github.com/ismaeeeelshaikh/college-ai-assistant)

---

### 🌐 DevOps Club Official Website
> **Production platform handling 96+ hackathon participants**

- ⚛️ React + Firebase real-time backend
- 🔄 Automated CI/CD pipelines — zero downtime under peak traffic
- 🎯 Handled registrations for 96+ participants (24 teams)

**🔗 Live:** https://devopsclub.apsit.edu.in/

---

## 📊 GitHub Stats

<div align="center">

![Ismaeel's GitHub Stats](https://github-readme-stats.vercel.app/api?username=ismaeeeelshaikh&show_icons=true&theme=tokyonight&hide_border=true&count_private=true)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=ismaeeeelshaikh&layout=compact&theme=tokyonight&hide_border=true)

![GitHub Streak](https://streak-stats.demolab.com?user=ismaeeeelshaikh&theme=tokyonight&hide_border=true)

</div>

---

## 🏆 Achievements & Certifications

| 🏅 Achievement | Details |
|---|---|
| 🥇 **Shortlisted — AI for Bharat** | Google & Govt. of India National AI Initiative + $100 AWS Grant |
| 🥈 **First Runner-Up — Sparkathon** | Inter-collegiate hackathon @ APSIT |
| 🏆 **GDG Solution Challenge** | Certificate of Achievement — Google Developer Groups |
| 📜 **Elite NPTEL Certification** | Database Management Systems — IIT Kharagpur |
| 🤖 **IBM Agentic AI Architect** | From Learner to Builder |
| 🔐 **Cybersecurity Virtual Internship** | Network Security Associate |

---

## 🎯 Current Focus

- 🤖 Advanced Agentic AI Systems
- ☁️ Large-scale Serverless Architectures
- 🔍 RAG optimization & LLM fine-tuning
- 📚 DSA & System Design for Big Tech

---

## 🤝 Let's Connect

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/ismaeel-shaikh-imms)
[![Email](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:shaikhismaeel75123@gmail.com)

**Build systems that scale. Build AI that works.** 🚀

</div>

---


